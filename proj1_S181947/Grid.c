...
// Contient l'information d'une case de la grille.
struct Cell_t {
    int isMine; 
    int isRevealed;
    int isFlagged;
...
Grid *gridInit(int width, int height, int nbrBombs) {
	...
	srand((unsigned) time(NULL)); // Nécessaire pour le RNG
     ...
     */
  	grid->cells = malloc(((unsigned) width) * sizeof(Cell *));
  	if (grid->cells == NULL) {
    	printf("Erreur d'allocation mémoire. Arrêt du programme.");
    	exit(-1);
  	}
  	...
  	for (int i = 0; i < width; i++) {
    	grid->cells[i] = malloc(((unsigned) height) * sizeof(Cell));
